...
class Question extends React.Component<QuestionProps, QuestionState> {
  ...
  onPropositionClick(propositionIndex: number) {
    if (this.props.multipleChoices) {
      ...
      propositions[propositionIndex].selected =
        !propositions[propositionIndex].selected;
  ...
  onNextButtonClick = () => {
    const propositionsWithIndices = this.state.propositions.map(
      (proposition, index) => ({ ...proposition, index })
    );
  ...
  render() {
    const isNextButtonDisabled: boolean =
      this.state.propositions.filter((proposition) => proposition.selected)
        .length === 0;

    return (
      <div className="tpb-quizz-question" data-testid="question">
        ...
        <div className="tpb-quizz-question-propositions">
          {this.state.propositions.map((proposition, propositionIndex) => (
            <button
              className={`tpb-quizz-question-proposition ${
                proposition.selected ? "selected" : ""
              }`}
              key={proposition.key}
              onClick={() => this.onPropositionClick(propositionIndex)}
        ...
        {this.props.multipleChoices && (
          <button
            className={`tpb-quizz-question-next-button ${
